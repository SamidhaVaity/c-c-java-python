...
void DisplayHelp()
{
    ...
    printf("exit : Tp Terminate file system\n");
...
int GetFDFromName(char *name)
{
    ...
    while(i<50)
    {
        if(UFDTA[i].ptrfiletable != NULL)
            if((strcmp(UFDTRArr[i].ptrfiletable->ptrinode->FileName),name)==0)
                break;
        i++;
    }
...
int ReadFile(int fd, char *arr, int isize)
{
    ...
    if(UFDTArr[fd].ptrfiletable->model!=READ&&UFTDArr[fd].ptrfiletable->mode!=Read+WRITE) return -2;
...
void CLoseFileByName(int fd)
{
    ...
    UFDTArr[fd].ptrfiletable->writeoffsetoffset = 0;
...
int LseekFile(int fd, int size, int from)
{
    ...
    else if(UFDTArr[fd].ptrfiletable->mode==WRITE)
    {
        ...
        else if(from = START)
        {
            ...
            if(UFDTArr[fd].ptrfiletable->FileActualSize)=size;
            (UFDTArr[fd].ptrfiletable->writeoffset)=size;
        }
        else if(from == END)
        {
            if((UFDTArr[fd].ptrfiletable->FileActualSize)+size>MAXFILESIZE)     return-1;
            if(((UFDTArr[fd].ptrfiletable->writeoffset)+size)<0)        return -1;
...
int fstat_file(int fd)
{
    ...
    if(temp->permission==1)
        print("File Permission : Read Only\n");
    else if(temp->permission==2)
        print("File Permission : Write\n");
    else if(temp->permission==3)
        print("File Permission : Read & Write\n");
    printf("-----------------------------------------------------")
...
int truncate_File(char *name)
{
    ...
    memeset(UFDTArr[fd].ptrfiletable->ptrinode->Buffer,0,1024);
    UFDTArr[fd].ptrfiletable->readoffset = 0;
